(function(a){a(document).ready(function(){var d=a(".loading").fadeIn("fast"),h=a(window),c=a("#main");function f(i,j){var k=(j=="width")?h.width():h.height();return parseInt((k/2)-((parseInt(i.data(j))+40)/2),10)}function e(i,j){var k=i.children("img").hide().attr("src",j.attr("href"));a.merge(k,i).animate({width:j.data("width")+"px",height:j.data("height")+"px",left:f(j,"width"),top:f(j,"height")});k.imagesLoaded(function(){k.fadeIn("fast")})}function b(k){var j=a(k),i=a("<div/>").addClass("overlay").css({position:"absolute",width:j.data("width")+"px",height:j.data("height")+"px"}).appendTo("body").overlay({load:true,left:f(j,"width"),top:f(j,"height"),mask:{color:"#000",opacity:0.4,loadSpeed:200},onBeforeLoad:function(l){j.css("boxShadow","none")},onClose:function(l){i.remove()}});a("<img/>").attr("src",j.attr("href")).attr("width",j.data("width")).attr("height",j.data("height")).click(function(){j=j.parent().next().children("a");if(j.size()==0){j=g.filter(":first")}e(i,j)}).appendTo(i);return false}var g=c.find("a").click(function(){return b(this)});c.imagesLoaded(function(i){c.masonry({itemSelector:"li",columnWidth:25});d.fadeOut("fast",function(){c.find("li").addClass("show").fadeIn("fast");a("#main").infinitescroll({navSelector:".next",nextSelector:".next",itemSelector:"#main li",bufferPx:100,loading:{finishedMsg:"",img:"/static/img/loading.gif",msgText:"",},pathParse:{join:function(j){return"/"+j}}},function(j){var k=a(j).addClass("show").hide();k.find("a").click(function(){return b(this)});k.imagesLoaded(function(){c.masonry("appended",k,true);k.fadeIn("fast")})})})})})})(jQuery);
/*!
